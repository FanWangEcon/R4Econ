...
description: |
  ...
  Regress binary preterm outcome on percentage of days exposed to extreme temperature, controlling for month of conception fixed effects.
...
core:
  - package: stats
    code: |
      ...
      rnorm()
      runif()
      lm(binary ~ continuous + factor(dates))
  - package: ggplot
    code: |
      geom_point()
      geom_bar()
...
##### Regression pre-term Births and Extreme Temperature Exposures

In the four datasets that we have generated, each under a different set of conception date distribution assumptions, we have information on the binary prer-term outcome and extreme cold exposure for each individual. We now regress pre-term on extreme cold exposures.

In the regression under **Scenarior (A)**, percent of extreme cold days is unrelated to pre-term.
```{r}
