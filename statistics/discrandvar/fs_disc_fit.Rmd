...
\mathcal{L}(\theta) = 
    \Pi_{i=0}^N    
    \left(
    \frac{N!}{\left(N-x_i\right)!x_i!}
    \cdot \theta^{x_i} \cdot \left(1-\theta\right)^{N-x_i}
    \right)^{P(x_i)}
\end{aligned}
$$
...
\sum_{i=0}^N
\left(
P(x_i)\ln\left(\frac{N!}{\left(N-x_i\right)!x_i!}\right) +
P(x_i) x_i \ln\left(\theta\right) + 
P(x_i)(N-x_i)\ln\left(1-\theta\right)
\right)
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\sum_{i=0}^N P(x_i)x_i}{\theta} &=
\frac{\sum_{i=0}^N P(x_i)(N-x_i)}{1-\theta}\\
\sum_{i=0}^N P(x_i)x_i - \theta \cdot \sum_{i=0}^N P(x_i)x_i &=
\theta \cdot \sum_{i=0}^N P(x_i)(N-x_i)\\
\theta &= \frac{
\sum_{i=0}^N P(x_i)x_i
}{
\sum_{i=0}^N 
\left(P(x_i)(N-x_i) + P(x_i)x_i\right)
}\\
\theta &= \frac{
\sum_{i=0}^N P(x_i)x_i
}{
...
#### Estimate Change of Success and the Number of Trials
...
Sixth, we now proceed to find the optimum $M$ value, assuming that multiple probability vectors can have different $\theta$ but must share $M$. We find the optimizing $\theta$ that differs for each of the discrete probability vectors.
...
Seventh, we evaluat the predicted probabilities given our optimizing parameters. 
