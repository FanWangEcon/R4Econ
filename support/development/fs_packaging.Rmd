...
```
C:\Users\fan\R4Econ\support\development
```
...
## Coding Conventions inside File

### Message, Warning, Verbose

Functions should generally print key messages as *message*, and extra outputs that show full results, full df outputs etc, or intermediary outputs, that are not core information that are shown quickly, but details regarding outputs should be turned on and off by *bl_verbose* parameter. 
...
1. Generate RMD files first, to work through entire procedures line by line and understand details and show outputs point by point. 
2. *curl* file to R.
3. functionalize componenents, continuous run and test, divide ingredients into separate functions.
4. document and save as */R* folder file.
...
# setwd('C:/Users/fan/PrjOptiAlloc')
# setwd('G:/repos/PrjOptiAlloc')
# setwd('G:/repos/REconTools')
setwd('C:/Users/fan/REconTools')
devtools::document()
# Step 4
devtools::run_examples()
devtools::run_examples(pkg='C:/Users/fan/REconTools', start='ff_sup_git_status_check')
# devtools::build_vignettes()
# Step 5
# For this to work, functions that are used need to be EXPORTED
# Add EXPORT line to the function. Add minimal Function Descritpiosn
devtools::check()
...
During development, after new R functions have been written, testing latested functions in vignette. Note that this simulates realoading files in R folder. When building package and checking, need to do *devtools::install*.
...
usethis::use_build_ignore(c("_img", "code_r", "docs", "vignettes_working", "_pkgdown.yml"))
```

### Debuging and Errors

While Checking Package, often face errors. Here is a list of lessons:

1. read messages clearly, usually they are very informative, what line of code, which function has error
2. do not load tidyverse, tidymodels etc in vignette or functions, use specific packages. Add the packages, whether used in vignette or functions with *usethis::use_package("package_name")*.
3. do not add another personal package as a dependency, because that package will likely depend on REconTools, if dealing with REconTools
...
Restarted, and re-run, error seems to disappear without changing anything

### Pandoc 

> Error: pandoc document conversion failed with error 83

version issue with pandoc-citetproc and pandoc, rersolved by upgrading to rmarkdown_2.5. 
